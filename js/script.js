...
function showToast(message) {
    ...
    if (!toast) {
        ...
        toast.className = 'fixed bottom-20 right-6 z-[99999] bg-dark text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 text-sm font-medium border border-border toast-animate';
...
function initBookingModal() {
    ...
    const modalTriggers = document.querySelectorAll('[data-open-modal="booking"], .open-booking-btn, a[href="#bookingModal"]');
    const closeTriggers = modal.querySelectorAll('[data-close-modal="booking"], .modal-close-btn');
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', (e) => {
            ...
            if (successBox) {
                ...
                alert('Your appointment request has been submitted successfully! Our dermatologist coordinator will call you shortly.');
                closeBookingModal();
            }
        });
    }
...
window.openBookingModal = function(treatment = '') {
    ...
    if (treatmentSelect && treatment) {
        ...
        for (let opt of treatmentSelect.options) {
            if (opt.value.toLowerCase() === treatment.toLowerCase() || opt.text.toLowerCase().includes(treatment.toLowerCase())) {
...
// 11. Multi-Step Booking Page (booking.html)
function initBookingMultiStep() {
    ...
    function goToStep(stepNumber) {
        ...
        stepContents.forEach(content => {
            const contentStep = parseInt(content.getAttribute('data-step-content'), 10);
...
// 13. Mobile Bottom Navigation active status
